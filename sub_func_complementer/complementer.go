...
func (c *SubFuncComplementer) ComplementHeader(input *dpfm_api_input_reader.SDC, subfuncSDC *SDC, l *logger.Logger) error {
	s := &SDC{}
	numRange, err := c.ComplementOrderID(input, l)
	if err != nil {
		return xerrors.Errorf("complement orderID error: %w", err)
	}
...
func (c *SubFuncComplementer) ComplementItem(input *dpfm_api_input_reader.SDC, subfuncSDC *SDC, l *logger.Logger) error {
	...
	subfuncSDC.Message.ItemScheduleLine = msg.ItemScheduleLine
...
func (c *SubFuncComplementer) ComplementOrderID(input *dpfm_api_input_reader.SDC, l *logger.Logger) (*NumberRange, error) {
	rows, err := c.db.Query(
		...
		WHERE (ServiceLabel, FieldNameWithNumberRange) = ( (?, 'OrderID') );`, input.ServiceLabel,
	)
	if err != nil {
	...
	nr.LatestNumber++
	input.Header.OrderID = nr.LatestNumber
	return &nr, nil
}
