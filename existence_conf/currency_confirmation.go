...
func (c *ExistenceConf) currencyExistenceConf(mapper ExConfMapper, input *dpfm_api_input_reader.SDC, existenceMap *[]bool, exconfErrMsg *string, errs *[]error, mtx *sync.Mutex, wg *sync.WaitGroup, log *logger.Logger) {
	defer wg.Done()
	wg2 := sync.WaitGroup{}
	exReqTimes := 0
	...
	for _, header := range headers {
		currency, err := getCurrencyExistenceConfKey(mapper, &header, exconfErrMsg)
		if err != nil {
...
func getCurrencyExistenceConfKey(mapper ExConfMapper, header *dpfm_api_input_reader.Header, exconfErrMsg *string) (string, error) {
	var currency string
	var err error
